...
(async () => {
  const siteId = 'communitysavings_bank';
  const expectedTitle = 'Community Savings Bank Assistant';
  ...
  try {
    ...
    // Since our widget runs in Shadow DOM inside the host page, we can query it directly
    // note: frameLocator is used when inside an iframe, but here it's directly in the page body
    ...
    await chatInput.fill('Hello! I want to ask about Community Savings Bank services.');
    ...
    if (messages.length > 1) {
      console.log('Test Passed Successfully! Chat widget is fully operational on Community Savings Bank.');
    } else {
    ...
    try {
      const failScreenshotPath = path.join(__dirname, '..', 'public', 'assets', 'csb-chat-failure.png');
